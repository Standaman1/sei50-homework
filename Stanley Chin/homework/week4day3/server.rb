...
# Optionally, but just for Ethan: Implement pagination for search results
...
get "/search" do
    erb :search_form
end #have search form from user to put movie into serverside and API to return the results of search


get "/search/results" do
    @results = HTTParty.get "https://api.themoviedb.org/3/search/movie?api_key=24d863d54c86392e6e1df55b9a328755&query=#{params[:titlename]}"
    erb :results
end #gets title from movie database and input into results



get "/movie/:id" do
#HTTParty replace id key with params id then erb into a movie file and @movie 
end